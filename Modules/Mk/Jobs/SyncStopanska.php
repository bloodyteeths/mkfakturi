...
/**
 * Sync Stopanska Bank Transactions Job
 *
 * Fetches transactions from Stopanska Bank via PSD2 API and stores them in database
 * Respects 15 req/min rate limit as noted in roadmap
 */
class SyncStopanska implements ShouldQueue
{
    ...
    {
        Log::info('Starting Stopanska Bank sync', [
        ...
        try {
            ...
            $tokens = $authController->getStoredTokens('stopanska', $this->companyId);

            if (! $tokens) {
                throw new \Exception('No Stopanska bank connection found for company '.$this->companyId);
            }

            // Check if token is expired
            if ($tokens['expires_at'] && Carbon::parse($tokens['expires_at'])->isPast()) {
                throw new \Exception('Stopanska bank token has expired for company '.$this->companyId);
            }

            // Initialize Stopanska gateway
            $gateway = new StopanskaGateway;
            ...
            if (empty($accounts)) {
                Log::warning('No accounts found for Stopanska sync', ['company_id' => $this->companyId]);
            ...
            Log::info('Stopanska Bank sync completed', [
                'company_id' => $this->companyId,
                'total_synced' => $totalSynced,
            ]);

        } catch (\Exception $e) {
            Log::error('Stopanska Bank sync failed', [
                'company_id' => $this->companyId,
                'error' => $e->getMessage(),
     ...
     */
    protected function syncAccountTransactions(StopanskaGateway $gateway, $account): int
    {
        try {
    ...
    protected function findOrCreateBankAccount($account): BankAccount
    {
        ...
        if (! $bankAccount) {
            $bankAccount = BankAccount::create([
                ...
                'account_name' => $account->getName() ?? 'Stopanska Account',
    ...
    {
        Log::error('Stopanska sync job failed permanently', [
            'company_id' => $this->companyId,
