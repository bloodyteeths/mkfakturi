...
class KomerAccountDetail
{
    ...
    {
        return $this->data['bic'] ?? 'KOBMKM22XXX';
    }
...
/**
 * Simple Transaction object for Komercijalna PSD2 responses
 */
class KomerTransaction
{
    protected $data;
...
class KomerGateway
{
    ...
    /**
     * Retrieve OAuth tokens from Komercijalna Banka using client credentials flow
     */
    public function retrieveTokens(string $clientId, string $clientSecret)
    {
        ...
        $response = $this->httpClient->post($this->getAccessTokenUrl(), [
            'headers' => [
                ...
                'User-Agent' => 'MKAccounting/1.0 KomerGateway',
            ],
            'form_params' => [
    ...
    /**
     * Get account details from Komercijalna Banka
     */
    public function getAccountDetails(): array
    {
        ...
        Log::info('Komercijalna account details retrieved', ['account_count' => count($accounts)]);

        return $accounts;
    }

    /**
     * Get SEPA transactions from Komercijalna Banka
     */
    public function getSepaTransactions(int $page = 1, int $limit = 100): array
    {
        ...
        foreach ($bookedTransactions as $txData) {
            $transaction = new KomerTransaction([
                ...
                'transactionUid' => $txData['entryReference'] ?? $txData['transactionId'] ?? uniqid('kb_'),
        ...
        Log::info('Komercijalna transactions retrieved', [
    ...
    protected function makeApiRequest(string $method, string $url, array $options = [])
    {
        ...
        while ($attempts < self::MAX_RETRY_ATTEMPTS) {
            try {
                ...
                Log::warning('Komercijalna API request failed', [
                    'method' => $method,
                    'url' => $url,
                    'attempt' => $attempts,
        ...
        // All attempts failed
        Log::error('Komercijalna API request failed after all retries', [
            'method' => $method,
            'url' => $url,
            'attempts' => $attempts,
    ...
    /**
     * Generate unique request ID for KB API calls
     */
    ...
    /**
     * KB-specific error handling
     */
    protected function handleApiError(\Exception $e): void
    {
        Log::error('Komercijalna Banka API Error', [
    ...
    public function getSandboxTestData(): array
    {
        ...
        return [
            'accounts' => [
                [
                    'resourceId' => 'KB_SANDBOX_001',
                    'iban' => 'MK07150000000000001',
                    'currency' => 'MKD',
                    'name' => 'KB Sandbox Test Account',
    ...
    {
        $companies = [
            'Business Solutions DOOEL',
            'Express Logistics DOO',
    ...
    public function getSandboxAccountsAndTransactions(): array
    {
        ...
        foreach ($testData['accounts'] as $accountData) {
            $accounts[] = new KomerAccountDetail([
                'id' => $accountData['resourceId'],
        ...
        foreach ($testData['transactions']['booked'] as $txData) {
            $transactions[] = new KomerTransaction([
    ...
    /**
     * Test connection and retrieve transactions for SB-04 completion
     */
    public function testConnectionAndRetrieveTransactions(): array
    {
        ...
        Log::info('Testing Komercijalna connection for SB-04 completion');
