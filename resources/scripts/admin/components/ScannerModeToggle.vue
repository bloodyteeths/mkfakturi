<template>
  <div>
      ...
      <div class="flex items-center space-x-3">
        ...
        <div>
          ...
          <button
            v-if="!isEnabled"
            type="button"
            class="text-xs underline mt-0.5 text-gray-400 hover:text-gray-600"
            @click.stop="showHelp = !showHelp"
          >
            {{ showHelp ? $t('general.close') : $t('stock.how_it_works') }}
          </button>
        </div>
      </div>
    ...
    <div
      v-if="showHelp && !isEnabled"
      class="px-4 py-3 bg-blue-50 border border-t-0 border-blue-200 rounded-b-lg text-xs text-blue-700 leading-relaxed"
    >
      {{ $t('stock.scanner_mode_help') }}
    </div>
  </div>
</template>
