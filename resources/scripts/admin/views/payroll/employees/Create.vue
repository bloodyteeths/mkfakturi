...
async function loadEmployee() {
  ...
  try {
    const response = await axios.get(`/api/v1/admin/payroll/employees/${route.params.id}`)
  ...
  try {
    ...
    if (isEdit.value) {
      response = await axios.put(
        `/api/v1/admin/payroll/employees/${route.params.id}`,
        currentEmployee
      )
    } else {
      response = await axios.post('/api/v1/admin/payroll/employees', currentEmployee)
    }
