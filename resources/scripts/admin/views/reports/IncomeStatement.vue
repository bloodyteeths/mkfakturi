<template>
  <div class="grid gap-8 md:grid-cols-12 pt-10">
    <div class="col-span-8 md:col-span-4">
      <BaseInputGroup
        :label="$t('reports.accounting.date_range')"
      ...
      <div class="flex flex-col mt-6 lg:space-x-3 lg:flex-row">
        <BaseInputGroup :label="$t('reports.accounting.from_date')">
        ...
        <BaseInputGroup :label="$t('reports.accounting.to_date')">
...
async function exportReport(format) {
  ...
  try {
    const response = await window.axios.get(
      `/api/v1/reports/export/income-statement/${getSelectedCompany.value.unique_hash}`,
      {
        params: {
    ...
    const filename = `income_statement_${fromDate}_${toDate}.${extension}`
