<template>
  <BasePage>
    <BasePageHeader :title="t('edit_compensation')">
      <BaseBreadcrumb>
        <BaseBreadcrumbItem :title="$t('general.home')" to="dashboard" />
        <BaseBreadcrumbItem :title="t('title')" to="../../compensations" />
        <BaseBreadcrumbItem :title="t('edit_compensation')" to="#" active />
      </BaseBreadcrumb>
    </BasePageHeader>

    <!-- Loading -->
    <div v-if="isLoadingCompensation" class="bg-white rounded-lg shadow p-6">
      <div class="space-y-4">
    ...
    <!-- Not draft warning -->
    <div v-else-if="compensation && compensation.status !== 'draft'" class="bg-yellow-50 rounded-lg border border-yellow-200 p-6 text-center">
      <BaseIcon name="ExclamationTriangleIcon" class="h-12 w-12 text-yellow-500 mx-auto" />
      <p class="mt-2 text-sm text-yellow-700">Only draft compensations can be edited.</p>
    </div>

    <template v-else-if="compensation">
      <!-- Step Indicator -->
      <div class="mb-8">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-white rounded-lg shadow">
              <div class="px-4 py-3 bg-blue-50 border-b border-blue-200 rounded-t-lg">
                ...
                <p class="text-xs text-blue-600">{{ t('total_receivables') }}: {{ formatMoney(receivablesSelectedTotal) }}</p>
              </div>
              <div v-if="eligibleReceivables.length === 0" class="p-6 text-center text-sm text-gray-500">
                {{ t('no_eligible_documents') }}
              </div>
              <div v-else class="divide-y divide-gray-100">
                <div v-for="doc in eligibleReceivables" :key="`r-${doc.id}`" class="p-3 hover:bg-gray-50">
            ...
            <div class="bg-white rounded-lg shadow">
              <div class="px-4 py-3 bg-amber-50 border-b border-amber-200 rounded-t-lg">
                ...
                <p class="text-xs text-amber-600">{{ t('total_payables') }}: {{ formatMoney(payablesSelectedTotal) }}</p>
              </div>
              <div v-if="eligiblePayables.length === 0" class="p-6 text-center text-sm text-gray-500">
                {{ t('no_eligible_documents') }}
              </div>
              <div v-else class="divide-y divide-gray-100">
                <div v-for="doc in eligiblePayables" :key="`p-${doc.id}`" class="p-3 hover:bg-gray-50">
                  <div class="flex items-center space-x-3">
                    <input
                      type="checkbox"
...
const isLoadingCompensation = ref(false)
...
const receivableSelections = ref([])
...
async function fetchCompensation() {
  ...
  isLoadingCompensation.value = true
  try {
    ...
    if (compensation.value) {
      // Pre-populate form
      form.customer_id = compensation.value.customer_id
      form.supplier_id = compensation.value.supplier_id
      form.compensation_date = compensation.value.compensation_date
        ? (typeof compensation.value.compensation_date === 'string'
          ? compensation.value.compensation_date.substring(0, 10)
          : compensation.value.compensation_date)
        : ''
      form.type = compensation.value.type || 'bilateral'
      form.notes = compensation.value.notes || ''

      // Pre-populate selections from existing items
      const items = compensation.value.items || []
      receivableSelections.value = items
        ...
        .map(i => ({
          ...
          due_amount: i.document_total,
          amount_offset: i.amount_offset,
          side: 'receivable',
        }))
      payableSelections.value = items
    ...
    isLoadingCompensation.value = false
  }
}
...
async function updateCompensation() {
  ...
  try {
    const response = await window.axios.put(`/compensations/${compensation.value.id}`, {
    ...
    notificationStore.showNotification({
      type: 'success',
      message: response.data?.message || t('updated_success') || 'Compensation updated',
    })

    router.push({ path: `/admin/compensations/${compensation.value.id}` })
  } catch (error) {
    notificationStore.showNotification({
      type: 'error',
      message: error.response?.data?.message || t('error_updating') || 'Failed to update compensation',
    })
  } finally {
...
// Lifecycle
onMounted(async () => {
  await Promise.all([fetchCompensation(), fetchCustomers(), fetchSuppliers()])
})
</script>
