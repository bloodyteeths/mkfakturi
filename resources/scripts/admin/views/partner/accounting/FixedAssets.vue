<template>
  <BasePage>
    <BasePageHeader :title="$t('partner.accounting.fixed_assets', 'Fixed Assets Register')">
      <template #actions>
        <BaseButton v-if="selectedCompanyId" variant="primary" @click="showCreateModal = true">
    ...
    <!-- Empty State -->
    <div v-else-if="hasSearched && assets.length === 0 && !registerData" class="bg-white rounded-lg shadow p-12 text-center">
...
async function loadAssets() {
  ...
  try {
    ...
    const response = await window.axios.get(`/partner/companies/${selectedCompanyId.value}/accounting/fixed-assets`, { params })
...
async function saveAsset() {
  ...
  try {
    ...
    if (showEditModal.value && selectedAsset.value) {
      await window.axios.put(`/partner/companies/${selectedCompanyId.value}/accounting/fixed-assets/${selectedAsset.value.id}`, payload)
      notificationStore.showNotification({ type: 'success', message: t('accounting.fixed_assets.updated', 'Asset updated') })
    } else {
      await window.axios.post(`/partner/companies/${selectedCompanyId.value}/accounting/fixed-assets`, payload)
...
async function disposeAsset() {
  if (!selectedAsset.value || !selectedCompanyId.value) return
  isDisposing.value = true
  try {
    await window.axios.post(`/partner/companies/${selectedCompanyId.value}/accounting/fixed-assets/${selectedAsset.value.id}/dispose`, disposeForm.value)
