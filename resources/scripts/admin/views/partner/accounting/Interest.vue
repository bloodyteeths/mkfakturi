<template>
  <BasePage>
    ...
    <div v-if="!selectedCompanyId" class="text-center py-12 bg-white rounded-lg shadow">
      <BaseIcon name="BuildingOfficeIcon" class="mx-auto h-12 w-12 text-gray-400" />
      <p class="mt-2 text-sm text-gray-500">{{ $t('partner.select_company_placeholder') }}</p>
    </div>
...
async function fetchCalculations(page = 1) {
  ...
  try {
    ...
    const response = await window.axios.get(partnerApi(''), { params })
...
async function fetchSummary() {
  try {
    const response = await window.axios.get(partnerApi('/summary'))
...
async function fetchCustomers() {
  try {
    const response = await window.axios.get(partnerApi('/customers'))
...
async function saveRate() {
  ...
  try {
    await window.axios.post(partnerApi('/rate'), { annual_rate: rate })
...
async function resetRate() {
  isSavingRate.value = true
  try {
    await window.axios.delete(partnerApi('/rate'))
...
function exportCsv() {
  ...
  link.href = url
  link.download = `interest-${selectedCompanyId.value}-${new Date().toISOString().slice(0,10)}.csv`
  link.click()
...
async function batchCalculate() {
  ...
  try {
    ...
    const response = await window.axios.post(partnerApi('/calculate'), payload)
...
async function generateNote() {
  ...
  try {
    const response = await window.axios.post(partnerApi('/generate-note'), {
...
async function sendNote() {
  ...
  try {
    const response = await window.axios.post(partnerApi('/send-note'), {
      customer_id: payload.customer_id,
      calculation_ids: payload.calculation_ids,
    })
