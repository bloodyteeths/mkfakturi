...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return buildArticleMetadata(locale, '/blog/recurring-invoices-mk', {
    title: {
...
export default async function RecurringInvoicesMkPage({
  ...
  const articleLd = articleJsonLd({
    ...
    datePublished: '2026-02-09',
    tags: ['recurring invoices', 'automation', 'subscriptions', 'Facturino'],
  })

  const breadcrumbLd = breadcrumbJsonLd([
    { name: homeLabel, href: `/${locale}` },
    { name: blogLabel, href: `/${locale}/blog` },
    { name: t.title, href: `/${locale}/blog/recurring-invoices-mk` },
  ])
