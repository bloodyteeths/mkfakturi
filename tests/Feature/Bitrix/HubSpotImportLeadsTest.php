...
/**
 * HubSpotImportLeads Feature Tests
 *
 * Tests the HubSpot lead import command functionality including:
 ...
 * - Skip-HubSpot flag
 *
 * @ticket HUBSPOT-04 - Lead Import Command
 */
class HubSpotImportLeadsTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_csv_import_creates_leads()
    {
        ...
        $this->artisan('hubspot:import-leads', ['--csv' => $this->testCsvPath, '--skip-hubspot' => true])
    ...
    public function test_invalid_csv_handling_missing_required_columns()
    {
        ...
        $this->artisan('hubspot:import-leads', ['--csv' => $this->testCsvPath])
    ...
    {
        $this->artisan('hubspot:import-leads', ['--csv' => '/nonexistent/path/leads.csv'])
    ...
    {
        $this->artisan('hubspot:import-leads')
    ...
    public function test_dry_run_flag_does_not_create_leads()
    {
        ...
        $this->artisan('hubspot:import-leads', ['--csv' => $this->testCsvPath, '--dry-run' => true])
    ...
    /** @test */
    public function test_import_with_hubspot_sync()
    {
        // Mock HubSpot API for contact, company, and deal creation
        Http::fake([
            ...
            'https://api.hubapi.com/crm/v3/objects/companies/search' => Http::response([
                'total' => 0,
                'results' => [],
            ], 200),

            // Company create
            'https://api.hubapi.com/crm/v3/objects/companies' => Http::response([
                'id' => '601',
                'properties' => ['domain' => 'hubspot.com'],
            ], 201),

            // Deal create
            'https://api.hubapi.com/crm/v3/objects/deals' => Http::response([
                'id' => '701',
                'properties' => ['dealname' => 'Partner: HubSpot Company'],
            ], 201),

            // Associations
            'https://api.hubapi.com/crm/v3/objects/*/associations/*' => Http::response([], 200),
        ]);

        $this->createTestCsv([
            ['HubSpot Company', 'hubspot@example.com', '+389701234567', 'Skopje', 'https://hubspot.com', 'api', '', 'Contact Name'],
        ]);
        ...
        // Verify HubSpot API was called to create contact
        Http::assertSent(function ($request) {
            return str_contains($request->url(), 'crm/v3/objects/contacts') &&
                   $request->method() === 'POST' &&
                   ($request['properties']['email'] ?? null) === 'hubspot@example.com';
        });
    }

    /** @test */
    public function test_skip_hubspot_flag_only_imports_locally()
    {
        Http::fake();
        ...
        $this->artisan('hubspot:import-leads', ['--csv' => $this->testCsvPath, '--skip-hubspot' => true])
            ->expectsOutputToContain('SKIP HUBSPOT MODE')
            ->expectsOutputToContain('Created: 1')
            ->assertSuccessful();
        ...
        // HubSpot API should not be called
    ...
    /** @test */
    public function test_import_fails_without_hubspot_config_when_not_skipping()
    {
